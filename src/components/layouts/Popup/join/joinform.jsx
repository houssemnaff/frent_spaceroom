    ...
    try {
      ...
      if (error.response) {
        ...
        if (errorMessage === "Vous avez déjà rejoint ce cours") {
          toast.error("Vous avez déjà rejoint ce cours");
        } else if (errorMessage === "Vous ne pouvez pas rejoindre votre propre cours") {
          toast.error("Vous ne pouvez pas rejoindre votre propre cours");
    ...
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="w-full p-6 bg-white shadow-md rounded-lg">
        ...
        <div className="flex justify-center gap-4 mt-4">
          ...
          <button
            type="button"
            className={`px-4 py-2 rounded-lg text-sm ${mode === "link" ? "bg-blue-500 text-white" : "bg-gray-200 text-gray-700"}`}
