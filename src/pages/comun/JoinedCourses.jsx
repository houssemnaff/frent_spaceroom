  ...
  useEffect(() => {
    const fetchCourses = async () => {
      try {
        ...
        // Récupérer les cours joints par l'utilisateur
        const joinedCoursesRes = await axios.get(`${import.meta.env.VITE_API_URL}/course/my-join-courses`, { headers });
        console.log("Mes cours joints", joinedCoursesRes.data.joinedCourses);
        setJoinedCourses(joinedCoursesRes.data.joinedCourses || []);
      } catch (error) {
...
console.log("user owwner join",user._id);
  if (loading) {
  ...
  const allCourses = [ ...joinedCourses];
